...
func (c *Client) GetMatchData(ctx context.Context, matchID int) (*MatchResponse, error) {
	if c.token == "" {
		return nil, fmt.Errorf("token is empty")
	}
	params := url.Values{}
	params.Add("match_id", fmt.Sprintf("%d", matchID))
	reqUrl := fmt.Sprintf("%s/league_match_get?%s", c.baseUrl, params.Encode())
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, reqUrl, nil)
	if err != nil {
		return nil, err
	}
